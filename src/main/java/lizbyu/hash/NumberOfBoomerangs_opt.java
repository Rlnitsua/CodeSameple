...
public class NumberOfBoomerangs_opt {
    private static final String TAG = "NumberOfBoomerangs_opt";

    public int numberOfBoomerangs(int[][] points) {
        int boomerCount = 0;
        int numberSize = points.length;
        Map<Integer, Integer> map = new HashMap<Integer, Integer>();

        // traverse all points
        for (int i = 0; i < numberSize; i++) {
            // here is solution to calculate one point's boomerCount
            // record all different distances for p[i](one point)
            for (int j = 0; j < numberSize; j++) {
                int distance = calculateDist(points[i], points[j]);
                int distanceCount = map.get(distance) == null ? 0 : map.get(distance);
                map.put(distance, ++distanceCount);
            }

            // for one distance, boomerCount is equal to An2 = n(n-1)
            // traverse all values to calculate all distance
            for (int distanceCount : map.values()) {
                boomerCount += distanceCount * (distanceCount - 1);
            }

            // reset map's values and calculate next point
            map.clear();
        }
        return boomerCount;
    }
