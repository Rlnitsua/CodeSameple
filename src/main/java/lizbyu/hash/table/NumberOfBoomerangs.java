...
public class NumberOfBoomerangs {
	private static final String TAG = "NumberOfBoomerangs";
	...
	private int[][] getDistances(int[][] points) {
		int maxSize = points.length;
		int[][] distances = new int[maxSize][maxSize];
	...
	private int calculateDistance(int[] point1, int[] point2) {
		...
		// delete useless calculate
//		return Math.sqrt( (py1 - py2) * (py1 - py2) + (px1 - px2) * (px1 - px2) );
		return (py1 - py2) * (py1 - py2) + (px1 - px2) * (px1 - px2);
	}
...
//		int[][] array = { { 0, 0 }, { 1, 0 }, { -1, 0 }, { 0, 1 }, { 0, -1 } };
		...
		int numberOfBoomerangs = new NumberOfBoomerangs().numberOfBoomerangs(array);
		LogUtils.d(TAG, numberOfBoomerangs);
		...
		LogUtils.d(TAG, "test get 3 : " + map.get(3));
	}
}
