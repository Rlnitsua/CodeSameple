...
public class AssignCookies {
	private static final String TAG = "AssignCookies";
	...
	public int findContentChildren(int[] g, int[] s) {
		...
		for(int childIndex = 0; childIndex < g.length; childIndex++){
			...
			if(g[childIndex] > s[cookiesIndex]){
				...
				for(int i = cookiesIndex; i < s.length; i++){
					if(g[childIndex] > s[cookiesIndex]){
						cookiesIndex++;
					}else {
						cookiesIndex++;
	...
	private void swapArray(int[] g, int j, int i) {
		int oldI = g[i];
		g[i] = g[j];
		g[j] = oldI;
	}
...
//		int[] cookies = {1, 1};
		...
		LogUtils.d(TAG, new AssignCookies().findContentChildren(children, cookies));
//		LogUtils.d(TAG, new AssignCookies().sortArray(children));
	}
}
